...
df_er['Smoothed Reward'] = df_er.groupby('Log')['Episodic Reward'].rolling(window=80, min_periods=1, closed='right').mean().values
df_cu['Smoothed Users'] = df_cu.groupby('Log')['Connected Users'].rolling(window=80, min_periods=1, closed='right').mean().values
...
# ax.set_title('Convergence with Connected Users')
ax.set_xlabel('Episode', fontsize = 14, family = 'Times New Roman')
